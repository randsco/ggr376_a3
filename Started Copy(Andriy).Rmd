...
krige_nb <- poly2nb(state_boundaries, dpad_krige, 2)
lcosts_krige <- nbcosts(krige_nb, dpad_krige)
krige_w <- nb2listw(krige_nb, lcosts_krige, style="B")
mst_krige <- mstree(krige_w,5)

par(mar=c(0,0,0,0))
plot(st_geometry(state_boundaries), border=gray(.5))
plot(mst_krige, coordinates(as(state_boundaries, "Spatial")), col=2, 
